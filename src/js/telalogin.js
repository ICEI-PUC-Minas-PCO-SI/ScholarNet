  ...
  fetch("http://localhost:3009/educadorData")
    ...
    .then((educadorData) => {
      ...
      for (let i = 0; i < educadorCredentials.length; i++) {
        if (
          educadorCredentials[i].Email === inputEmail &&
          educadorCredentials[i].Senha === inputPassword
        ) {
      ...
      fetch("http://localhost:3009/alunoData")
        ...
        .then((alunoData) => {
          ...
          for (let i = 0; i < alunoCredentials.length; i++) {
            if (
              alunoCredentials[i].Email === inputEmail &&
              alunoCredentials[i].Senha === inputPassword
            ) {
